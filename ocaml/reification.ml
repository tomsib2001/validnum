...
let rec elemFun_to_string soc = function
  ...
  | Sqrt x -> "( Sqrt ("^(elemFun_to_string soc x)^"))"
...
let makeF const var plus mult neg sub div sqrt cos sin exp log pow varfun f = 
  let rec aux = function
    ...
    | Sqrt f1 -> let if1 = aux f1 in (fun x -> sqrt (if1 x))
...
let sym2iFunFloat = makeF thin iVar iPlus iMult iNeg iSub iDiv iSqrt iCos iSin iExp iLog (fun (x,y) -> iPow x y) iVarFun;;

(* or more generally when it was of type 'a elemFun *)
let sym2iFunGen (aToIntervalle : 'a -> intervalle) = makeF aToIntervalle iVar iPlus iMult iNeg iSub iDiv iSqrt iCos iSin iExp iLog (fun (x,y) -> iPow x y) iVarFun;;
...
let sym2floatFun = makeF const var (+.) (fun x y -> x *. y ) (fun x -> ~-.x) (-.) (/.) sqrt cos sin exp log  (fun (x,y) -> pow x y) floatVarFun;;
...
let sqrtAD (a,b) = (iSqrt a, iMult (thin 0.5) (iDiv b (iSqrt a)));;
